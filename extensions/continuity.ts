...
export interface ContinuityCheckpointV1 {
	...
	checkpointId: string;
	createdAt: string;
	generated: ModelCheckpoint;
	effective: ModelCheckpoint;
	provenance: {
...
export function parseModelCheckpoint(text: string): ModelCheckpoint | undefined {
...
function copyDraft(model: ModelCheckpoint): ModelCheckpoint {
	return {
		...model,
		forbid: [...model.forbid],
...
function projectDraft(generated: ModelCheckpoint, overrides: Overrides): ModelCheckpoint {
	return {
		...copyDraft(generated),
		task: overrides.task ?? generated.task,
		doneWhen: overrides.doneWhen ?? generated.doneWhen,
		forbid: overrides.forbid ? [...overrides.forbid] : [...generated.forbid],
		status: overrides.status ?? generated.status,
	};
}
...
export function createCheckpoint(
	...
	const generated = copyDraft(model);
	return {
		...
		generated,
		effective: projectDraft(generated, overrides),
		provenance: {
			source: "continuity-model",
...
export function parseCheckpoint(value: unknown): ContinuityCheckpointV1 | undefined {
	...
	const generated = parseModelCheckpoint(JSON.stringify(value.generated));
	const effective = parseModelCheckpoint(JSON.stringify(value.effective));
	if (!generated || !effective || !isRecord(value.provenance) || !isRecord(value.authorization)) return undefined;
	if (
...
export function applyOverrides(
	...
	return {
		...checkpoint,
		generated: copyDraft(checkpoint.generated),
		effective: projectDraft(checkpoint.generated, overrides),
...
export function foldBranch(entries: readonly BranchEntry[]): FoldedState {
	...
	return checkpoint ? { checkpoint: applyOverrides(checkpoint, overrides), overrides } : { overrides };
}
...
function renderStatus(state: FoldedState): string {
	if (!state.checkpoint) {
		return `Continuity: no valid checkpoint yet. Effective user overrides: ${overrideNames(state.overrides)}.`;
	}
...
async function synthesize(
...
): Promise<{ checkpoint: ModelCheckpoint; usage: Usage } | undefined> {
	...
	try {
		...
		const text = response.content
			...
			.join("\n")
			.trim();
		const checkpoint = text ? parseModelCheckpoint(text) : undefined;
		return checkpoint ? { checkpoint, usage: response.usage } : undefined;
	} catch {
		return undefined;
	}
}
...
export function createContinuityExtension(dependencies: ContinuityDependencies) {
	return function continuityExtension(pi: ExtensionAPI): void {
		...
		pi.on("session_before_compact", async (event, ctx) => {
			...
			const checkpoint = createCheckpoint(synthesis.checkpoint, branchState.overrides, {
